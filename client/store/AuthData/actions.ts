...
export const loginUserAction = (params: TLoginUserParams) => {
  return async (dispatch: any) => {
    try {
      const { data } = await axios.post(`localhost/auth/login`, params);
  ...
  return async (dispatch: any) => {
    try {
      if (registrationData.password === registrationData.repeatPassword) {
        ...
        await axios.post(`localhost/auth/registration`, payload);
        dispatch(
          loginUserAction({
...
export const logoutUserAction = (token: string) => {
  return async (dispatch: any) => {
    try {
      await axios.post(`localhost/auth/logout`, { token });
...
export const getUserPermission = () => {
  return async (dispatch: any) => {
    ...
    try {
      const { data } = await axios.post(`localhost/auth/user-permission`, {
        token,
      });
      dispatch({
